...
for($i = 0;$i < count($campi);$i++)
 {
		...
		if(($campi[$i]["tipo"] != "submit"))
		{
			...
			if($campi[$i]["tipo"] == "textarea")
			{
				...
				{
					echo "<td>";
					echo "<textarea name=\"{$campi[$i]["nome"]}\" rows=\"{$campi[$i]["valore"][1]}\" cols=\"{$campi[$i]["valore"][2]}\">";
			...
			{	
				...
				if(($campi[$i]["tipo"] != "select"))
					if(($campi[$i + 1]["tipo"] != "submit"))
						echo "<td><input type=\"{$campi[$i]["tipo"]}\" name=\"{$campi[$i]["nome"]}\" id=\"{$campi[$i]["nome"]}\" value=\"{$campi[$i]["valore"]}\" class=\"{$classe_input}\" {$paggiuntivi} /></td>";
					else
						echo "<td colspan=\"" . ($campi_per_riga - $campo_riga) . "\"><input type=\"{$campi[$i]["tipo"]}\" name=\"{$campi[$i]["nome"]}\" id=\"{$campi[$i]["nome"]}\" value=\"{$campi[$i]["valore"]}\" class=\"{$classe_input}\" {$paggiuntivi}  /></td>";
					...
					{	unset($ke);
						...
						if(($campi[$i + 1]["tipo"] != "submit"))
							...
							echo "<td colspan=\"" . ($campi_per_riga - $campo_riga) ."\">\n";
						if($campi[$i]["multi"] == 1)
 							form_select($ke,$campi[$i]["nome"],$va,$classe_input,"",1);
						else
							form_select($ke,$campi[$i]["nome"],$va,$classe_input);
		...
		else
		{
				...
				form_invia($campi[$i]["valore"],$campi[$i]["nome"],"",$classe_input);
				echo "</td></tr>\n";
				$campo_riga = 0;
