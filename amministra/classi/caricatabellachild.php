...
$strtabella .= "<tr><th>{$campichilddesc[1]}</th><th>{$campiparentdesc[1]}</th><th>{$campichilddesc[2]}</th><th>{$campichilddesc[3]}</th>";
	...
	if(isset($_POST["nomecamposelecttabparent"]) && intval($_POST["nomecamposelecttabparent"]) > 0)
		$sql = "select {$tabchild}.{$campiid["tabchild"]} as idchild,{$tabchild}.*,{$tabparent}.* from {$tabchild},{$tabparent} where {$tabchild}.{$campo_chiave_esterna_child} = {$tabparent}.{$campiid["tabparent"]} and {$campo_chiave_esterna_child} = {$_POST["nomecamposelecttabparent"]} order by {$campo_chiave_esterna_child}";
	else
		$sql = "select {$tabchild}.{$campiid["tabchild"]} as idchild,{$tabchild}.*,{$tabparent}.* from {$tabchild},{$tabparent} where {$tabchild}.{$campo_chiave_esterna_child} = {$tabparent}.{$campiid["tabparent"]} order by {$campo_chiave_esterna_child}";
	$rs = esegui_query($con,$sql);
	$riga = 0;	
	while($r = getrecord($rs))
	{
